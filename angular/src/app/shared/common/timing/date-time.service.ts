...
export class DateTimeService {
    ...
    createDateRangePickerOptions(): any {
        let options = {
            ...
            min: DateTime('2015-05-01'),
            minDate: DateTime('2015-05-01'),
            max: DateTime(),
            maxDate: DateTime(),
            opens: 'left',
            ranges: {}
        ...
        options.ranges[this._appLocalizationService.l('Today')] = [DateTime().startOf('day'), DateTime().endOf('day')];
        options.ranges[this._appLocalizationService.l('Yesterday')] = [DateTime().minus({ days: 1 }).startOf('day'), DateTime().minus({ days: 1 }).endOf('day')];
        options.ranges[this._appLocalizationService.l('Last7Days')] = [DateTime().minus({ days: 6 }).startOf('day'), DateTime().endOf('day')];
        options.ranges[this._appLocalizationService.l('Last30Days')] = [DateTime().minus({ days: 29 }).startOf('day'), DateTime().endOf('day')];
        options.ranges[this._appLocalizationService.l('ThisMonth')] = [DateTime().startOf('month'), DateTime().endOf('month')];
        options.ranges[this._appLocalizationService.l('LastMonth')] = [DateTime().minus({ months: 1 }).startOf('month'), DateTime().minus({ months: 1 }).endOf('month')];
    ...
    getStartOfDayMinusDays(daysFromNow: number): DateTime {
        ...
        daysFromNow = this.minusDays(date, daysFromNow);
        return this.getEndOfDayForDate(daysFromNow);
    }
    ...
    getEndOfDayPlusDays(daysFromNow: number): DateTime {
        let date = this.getDate();
        daysFromNow = this.plusDays(date, daysFromNow);
        return this.getEndOfDayForDate(daysFromNow);
    }
    ...
    fromNow(date: DateTime | Date): string {
        ...
        return date.toRelative(this.getDate());
    }
}
